    ...
    class InstructionTranslator
    {
            ...
            protected void DynamicDispatch(IMethodReference method, IVariable receiver, MethodCallOperation operation, Func<IMethodReference, string> funcOnPotentialCallee)
            {
                ...
                if (operation == MethodCallOperation.Virtual)
                {
                    ...
                    if (calless.Count > 0)
                    {
                        if (method.ResolvedMethod != null && (method.ResolvedMethod.IsAbstract || method.ResolvedMethod.IsExternal))
                        {
                            ...
                            else
                            {
                                ...
                                AddBoogie(boogieGenerator.ElseIf(boogieGenerator.BinaryOperationExpression(boogieGenerator.ReadAddr(receiver), boogieGenerator.NullObject(), "!="), boogieGenerator.Assert("false")));
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            AddBoogie(boogieGenerator.If(boogieGenerator.BinaryOperationExpression(boogieGenerator.ReadAddr(receiver), boogieGenerator.NullObject(), "!="), boogieGenerator.Assert("false")));
                        }
        ...
        class SimpleTranslation : Translation
        {
            ...
            public override void Visit(SwitchInstruction instruction)
            {
                ...
                {
                    var indexCondition = boogieGenerator.BinaryOperationExpression(boogieGenerator.ReadAddr(instruction.Operand), idx.ToString(), "==");
                    var body = boogieGenerator.Goto(target);
                    sb.AppendLine(boogieGenerator.If(indexCondition, body));
                    idx++;
                }

                var cond = boogieGenerator.BinaryOperationExpression(boogieGenerator.ReadAddr(instruction.Operand), instruction.Targets.Count.ToString(), "<");
            ...
            public override void Visit(ReturnInstruction instruction)
            {
                ...
                if (instruction.HasOperand)
                    AddBoogie(boogieGenerator.VariableAssignment("$result", boogieGenerator.ReadAddr(instruction.Operand)));
            ...
            public override void Visit(LoadInstruction instruction)
            {
                ...
                else if (instructionOperand is StaticFieldAccess) // memory access handling
                {
                    ...
                    {
                        AddBoogie(boogieGenerator.VariableAssignment(FieldTranslator.GetFieldName(staticFieldAccess.Field), boogieGenerator.NullObject()));
                    }
            ...
            public override void Visit(CreateObjectInstruction instruction)
            {
                ...
                AddBoogie(bg.AssumeTypeConstructor(bg.ReadAddr(instruction.Result), typeString));
            }
            ...
            public override void Visit(StoreInstruction instruction)
            {
                ...
                {
                    var address = new AddressExpression(dereference.Type,boogieGenerator.ReadAddr(dereference.Reference), dereference.Reference);
            ...
            private void ProcessAs(ConvertInstruction instruction)
            {
                ...
                var bg = boogieGenerator;
                AddBoogie(bg.VariableAssignment(dest, bg.As(source, type)));
            }
        ...
        protected class ExceptionTranslation : Translation
        {
            ...
            public override void Visit(CatchInstruction instruction)
            {
                ...
                AddBoogie(boogieGenerator.VariableAssignment("$Exception", boogieGenerator.NullObject()));
                AddBoogie(boogieGenerator.VariableAssignment("$ExceptionType", boogieGenerator.NullObject()));
            }
            ...
            public override void Visit(ThrowInstruction instruction)
            {
                ...
                {
                    var args = new List<string>();
                    args.Add(boogieGenerator.ReadAddr(instruction.Operand));
                    AddBoogie(boogieGenerator.ProcedureCall("System.Object.GetType", args, "$ExceptionType"));
                    AddBoogie(boogieGenerator.VariableAssignment("$Exception", boogieGenerator.ReadAddr(instruction.Operand)));
                }
                else
                {
                    // rethrow
